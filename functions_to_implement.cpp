...
{
	size_t pos = s1.find(s2); // Initial position of the first occurrence

	// Use a loop to find and remove all occurrences of 's2' in 's1'
	while (pos != std::string::npos)
	{
		// If 's2' is found in 's1', erase it
		s1.erase(pos, s2.length());

		// Find the next occurrence of 's2' starting from the updated position
		pos = s1.find(s2, pos);
	}

	// Return the modified string s1 without occurrences from s2
	return s1;
}
...
std::string Join(std::vector<std::string> pieces, std::string glue)
{
	...
	string result; // result is the combined verctors

	// Iterate through all the vectors of strings
	for (size_t i = 0; i < pieces.size(); i++)
	{
		// Add the current string to the result
		result += pieces[i];
		// Add the glue string, if it's not the last string.
		if (i < pieces.size() - 1)
		{
			result += glue;
		}
...
std::vector<bool> EvenMask(std::vector<int> num)
{
	vector<bool> result;
